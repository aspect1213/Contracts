...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Договор_ЗаписьИзПочты" Тогда
		
		Идентификатор = Параметр.Идентификатор;
		ИдентификаторыФайлов = Параметр.ИдентификаторыФайлов;
		...
		Если Строки.Количество() = 1 Тогда
			Сообщение = Строки[0];
			ОбработатьПисьмоПослеЗагрузки(Сообщение, Сообщение.ПолучитьИдентификатор(), Ссылка, ИдентификаторыФайлов);
		КонецЕсли;
...
Процедура ВходящиеСообщенияВложенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Вложение.ПолноеИмяФайла) Тогда
		ДоговорныеДокументыКлиент.ОткрытьФайлНаДиске(Вложение.ПолноеИмяФайла, Вложение.Имя);
	ИначеЕсли ЭтоАдресВременногоХранилища(Вложение.Адрес) Тогда
		...
		#Иначе
			ДоговорныеДокументыКлиент.ОткрытьФайлИзВременногоХранилища(
...
Процедура ОбновитьСписокСообщений(Команда = Неопределено)
	...
	Если ВидПочтовогоКлиентаMSOutlook() Тогда
		ЭтаФорма.Заголовок 
			= СтрШаблон(НСтр("ru = 'Загрузка документов из Microsoft Outlook (папка: ""%1"")'"), 
				ПутьOutlook);
	Иначе	
		ЭтаФорма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Загрузка документов из почты %1'"),
			Профиль);
	КонецЕсли;	
...
Процедура СоздатьДокументИзСообщения()
	...
	Если Элементы.ВходящиеСообщения.ТекущиеДанные.Импортирован Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Все файлы письма уже загружены ранее.'"));
		Возврат;
	КонецЕсли;
	
	Если Элементы.ВходящиеСообщения.ТекущиеДанные.Вложения.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Это письмо не содержит файлов.'"));
		Возврат;
	КонецЕсли;
	
	МассивВыделенныеСтроки = ПолучитьВыделенныеСтроки();
	Если МассивВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите файлы для загрузки.'"));
		Возврат;
	КонецЕсли;
	...
	ИмяФайла = "";
	Если ЕстьИмпортированныеФайлы(Сообщение, ИмяФайла) Тогда
	...
	РанееЗагруженныеДокументы = НайтиРанееЗагруженныеДокументы(Сообщение.Отправитель);
	Если РанееЗагруженныеДокументы.Количество() <> 0 Тогда
		
		ПараметрыСообщения.Вставить("РанееЗагруженныеДокументы", РанееЗагруженныеДокументы);
		...
		ТекстВопроса = СтрШаблон(НСтр("ru = 'Вы уже ранее загружали документы от этого отправителя (%1).
			|Создать новый документ или обновить файлы в существующем?'"), Сообщение.Отправитель);
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("СоздатьНовый", НСтр("ru = 'Создать новый'"));
		Кнопки.Добавить("ОбновитьФайлы", НСтр("ru = 'Обновить файлы'"));
...
Процедура СоздатьДокументИзСообщенияПослеПроверкиПовторногоИмпортаДокумента(
	...
	Если Ответ = "ОбновитьФайлы" Тогда
		
		ПараметрыСообщения = Новый Структура;
		ПараметрыСообщения.Вставить("Сообщение", Параметры.Сообщение);
		
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ПослеВыбораДокументаДляОбновления",
			ЭтотОбъект,
			ПараметрыСообщения);			
		
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("Документы", Параметры.РанееЗагруженныеДокументы);
		
		ОткрытьФорму(
			"Справочник.ДоговорныеДокументы.ФормаВыбора",
			ПараметрыОткрытия,,,,,
			ОписаниеОповещения,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ПослеВыбораДокументаДляОбновления(Документ, Параметры) Экспорт
	...
	Если Строки.Количество() = 1 Тогда
		Сообщение = Строки[0];
		ОбработатьПисьмоПослеЗагрузки(Сообщение, Сообщение.ПолучитьИдентификатор(), Документ, ИдентификаторыФайлов);
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьПисьмоПослеЗагрузки(Сообщение, ТекущаяСтрокаСообщения, СсылкаНаДоговор, ИдентификаторыФайлов)
	...
	Если ВходящиеСообщенияСтрока <> Неопределено Тогда
		...
		ЗарегистрироватьФайлы(ИдентификаторыФайлов);
		
		Для Каждого ФайлСтрока Из ИдентификаторыФайлов Цикл
			
			Для Каждого Вложение Из ВходящиеСообщенияСтрока.Вложения Цикл
			
				Если Вложение.Идентификатор = ФайлСтрока.Идентификатор Тогда
					Вложение.Импортирован = Истина;
					Вложение.Имя = Вложение.Имя + " " + НСтр("ru ='(загружен)'");
				КонецЕсли;	
		...
		ЗагруженыВсеФайлы = Истина;
		Для Каждого Вложение Из ВходящиеСообщенияСтрока.Вложения Цикл
		
			Если Не Вложение.Импортирован Тогда
				ЗагруженыВсеФайлы = Ложь;
			КонецЕсли;	
				
		КонецЦикла;		
		
		Если ЗагруженыВсеФайлы Тогда
...
Функция НайтиКонтрагентаПоEmail(Текст)
	...
	Запрос.Текст = 
		...
		|	И Контрагенты.АдресЭлектроннойПочты ПОДОБНО &Текст";
