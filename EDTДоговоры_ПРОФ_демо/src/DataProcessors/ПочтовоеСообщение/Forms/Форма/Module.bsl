...
Процедура ЗаполнитьПолучателей(Объекты = Неопределено)
	...
	Если ТипЗнч(Объекты) = Тип("Массив") Тогда
		Для каждого Контрагент Из Объекты Цикл
...
Функция ПолучитьАдресДляОтправкиЛегкойПочтой(Получатель)
	...
	Если ТипЗнч(Получатель) = Тип("СправочникСсылка.Контрагенты") Тогда
		ТекстЗапроса =
			"ВЫБРАТЬ
			|	Контрагенты.АдресЭлектроннойПочты КАК ПочтовыйАдрес,
...
// Формирует список получателей, контрагентов и контактных лиц для автоподбора
